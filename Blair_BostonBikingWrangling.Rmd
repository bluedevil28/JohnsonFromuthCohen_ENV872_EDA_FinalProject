...
census.data.joined <- select(census.data.joined, -NAME.1, -NAME.2, -NAME.3)


bikes.data.join <- census.data.joined %>%
  select(NAME, Name, perwhite, pernonwhite, per.homeowner, per.public, estimate) %>%
  group_by(NAME, perwhite, pernonwhite, per.homeowner, per.public) %>%
...
```{r race and bikes join and correlation}
bike.variables.model <- lm(data=bikes.data.join, NumStations ~ perwhite + per.homeowner + per.public)
summary(bike.variables.model)
#R-squared of 0.1012, p-value 0.0001873



Bikes.AIC.1 <- lm (data=bikes.data.join, NumStations ~ perwhite + per.homeowner + per.public + pernonwhite) 
step(Bikes.AIC.1)

Bikes.AIC.2 <- lm (data=bikes.data.join, NumStations ~ perwhite + per.homeowner + per.public ) 
step(Bikes.AIC.2)
#<none> is first here 

Bikes.AIC.3 <-  lm (data=bikes.data.join, NumStations ~ pernonwhite + per.homeowner + per.public ) 
step(Bikes.AIC.3)
