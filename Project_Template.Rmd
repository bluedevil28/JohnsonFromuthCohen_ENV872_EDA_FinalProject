...
data.time.period.column.3 <- c("Released in 2021", "2016-2020 Census")
...
knitr::kable(Data.table, 
             caption = "Dataset Information", table.attri ="style='width:30%;")
```
...
```{r bike stations and census data loading, echo= FALSE, message=FALSE, warning=FALSE}
...
```{r map bike stations and census, echo= FALSE}

boston.income.sf2 <- st_read(
    "../JohnsonFromuthCohen_ENV872_EDA_FinalProject/Data_Processed/boston.income.shp")

#map income
ggplot(boston.income.sf2) + geom_sf(aes(fill=estimate)) + geom_sf(data=Bikes.2021.sf, aes(color="white", alpha = 0.4))
...
census_tracts$NAME20 <- as.numeric(census_tracts$NAME20)

#cannot round tracts

census_tracts$NAME20 <- round(census_tracts$NAME20)

#removed unpopulated tracts to allow bind

census_tracts.1 <- census_tracts %>% 
      filter(GEOID20 != "25025981202", GEOID20 != "25025981800", GEOID20 != "25025981300",
         GEOID20 != "25025990101", GEOID20 != "25025080801", 
         GEOID20 != "25025981100")

census_stations <- cbind(census_tracts.1, bikes.data.join)



#plot2 <-
 # ggplot(census_stations, aes(x=NAME20, y=NumStations)) +
  #geom_point(aes(color=NumStations)) +
  #ylab("Number Stations") +
  #xlab("Census Tract Number") +
  #xlim(0, 1500)
...
plot3 <-
  ggplot(bikes.data.join, aes(x=pernonwhite, y=NumStations)) +
  geom_point((aes(color=NumStations)), size = 3) +
#size = 0.1, alpha = .4), color="blue") +
...
plot3.1 <-
  ggplot(census.stations, aes(x=NumStations, y=pernonwhite)) +
  geom_boxplot(aes(size = 0.1, alpha = .4), color="blue") +
  ylab("Percent non white") +
  xlab("Number of stations")+
  xlim(1,6)
plot3.1

plot3.2 <-
  ggplot(census.stations) +
  geom_violin(aes(x = NumStations, y = pernonwhite), 
